...
# Example:
#   EPISODES_DIR=/mnt/media/southpark ./ingest_season.sh 5
...
BASE_DIR="${EPISODES_DIR:-/Users/pravinlohani/Downloads}"
SEASON_DIR="$BASE_DIR/Season $SEASON"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
BACKEND_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"  # Go up from scripts/ to backend/
...
python -u scripts/ingestion/ingest_audio_sequential.py "$SEASON_DIR" "$SEASON" 2>&1 | tee -a "ingest_season_${SEASON}.log"
